...
**Key features:** Double jump (with particle burst), 10 stages × 50 levels (500 total, **segment-composed** from seeded RNG — see Level generation below), shop with **buyable weapons + 5 upgrades** (Jump Boost, Speed Boots, Coin Magnet, DJ Boost, Extra Lives), 10 animated stage themes, **hazards** (spikes), **dynamic platforms** (moving + crumbling), **enemies + combat** (attack/punch — see below), **animated/interactive main menu** (see below).
...
**Physics constants:** `GRAVITY=1850`, `JUMP=555`, `DJ=495`, `SPEED=280 px/s`. `COYOTE_TIME=0.1s`, `JUMP_BUFFER=0.085s`. Max double-jump reach (base stats) ≈ 271px horizontal / 149px vertical. **Per-stage jump envelope (`stagePhys(stageIdx)` in level.js):** the generator scales its geometry to each stage's REAL physics — jump velocities scale by `jMul`, gravity by `gMul`, so horizontal reach scales by `jMul/gMul` and apex height by `jMul²/gMul`. `stagePhys` returns `{rise1, rise2, gapK, riseK}` (single/double-jump apex px and gap/rise multipliers, damped 15% toward 1 to keep the base safety margin); `buildLevel` stashes these on the builder (`b.gapK/b.riseK/b.rise1/b.rise2`) and every segment multiplies horizontal gaps by `gapK` and upward rises by `riseK` (drops stay unscaled — falling is free). So Space (reach ≈ 903px) gets ~2.8× wider gaps instead of letting a double jump skip 5 platforms, and Sky/Crystal/Meadow scale proportionally. `segGauntlet` derives its ceiling-spike height from `rise1`/`rise2` directly (single jump clears, double skewers — on every stage), `segSpikePath` widens spike beds by `gapK`, and `placeEnemies` scales the flyer `minGap` by `gapK`. The base constants are mirrored in level.js as `P_JUMP/P_DJ/P_GRAV` — keep in sync with player.js. Validated: exit reachable in all 500 levels under each stage's actual physics (trajectory-sim BFS).
...
**Level generation (`level.js`):** Levels are NOT a single random walk — they are **composed from curated segments** (set pieces), giving a deliberate, hand-built feel. `buildLevel()` walks a cursor `{x, y}` left→right and appends segments via `connect()` (which auto-arcs guide-coins over each jump and clamps required rise to `MAX_RISE=95 × riseK`). **Coin arcs match the real jump trajectory:** `arcCoins(coins, x1, y1, x2, y2, n, rng, phys)` samples `flightPath()` — a 120Hz simulation of the game's exact vertical integration (gravity, terminal velocity, double jump fired at the apex) under the stage's physics (`b.phys` from `stagePhys`, which exposes raw `g/j/dj/term`) — choosing a single jump when it covers both the height and distance, else jump+dj. Horizontal x advances linearly over the flight time (players modulate run speed), landing exactly on the target; y1/y2 are feet heights (platform tops) and coins sit ~26px above the foot path. There is no decorative `arcH` parameter anymore — `connect()` signature is `(b, gap, y, w, type, coinN)`. Arc: warm-up `segRest` → escalating segments paced with periodic breathers → a forced hard climax → `segFinish` (hosts the exit). Segment table `SEGMENTS[]` has `{fn, tier (0/1/2), gate}`; `pickSegment()` chooses by effective difficulty `e` (blends global `progress` + local position in the level) and `gate` (mechanic unlock thresholds: spikes ≥0.03, moving ≥0.06, ceiling-spike gauntlet ≥0.10, crumble ≥0.16, spike-leap ≥0.20). **Levels are 5× longer:** the full-curve length is `baseW = lerp(5500, 24000, progress)` (old was 1100→4800). **Opening on-ramp:** the first ~15 levels (start of Meadow) ease in via `introT = clamp(idx/15, 0, 1)` (idx = global level index): `targetW = lerp(1800, baseW, introT)` (much shorter early), effective difficulty is held down (`e *= 0.5 + 0.5*introT` → easy-tier segments), and the forced hard climax is eased (`climaxE *= 0.35 + 0.65*introT`) and **skipped entirely while `introT < 0.2`** (levels 1–3, so they stay ~2000px / 8 platforms and end gently). Result: L1–3 ≈ short & flat, enemies first appear ~L7 (`p≥0.012`), spikes ~L16 (gate 0.03), full curve by ~L16. Segment generators: `segRest, segGapRun, segStairsUp/Down, segPillars, segZigzag, segLongLeap, segDescentDrop, segSpikePath` (ground run w/ spike clusters), `segMovingBridge` (horizontal mover over void), `segElevator` (vertical lift), `segGauntlet` (ceiling spikes — single jump clears, double jump skewers), `segCrumbleRun, segSpikeLeap, segCoinVault` (reward set piece), `segFinish`.
**Level archetypes (`ARCHETYPES[]` + `archFor()` in level.js):** every non-boss level gets a seeded identity so levels read as distinct designs instead of uniform samples from one pool. 10 archetypes (RHYTHM, THE SUMMIT, THE DESCENT, THE RAVINE, MACHINEWORKS, SPIKE FIELDS, PRECISION, LEAP OF FAITH, GOLD RUSH, ROLLERCOASTER), each with: `wts` (per-segment-id weight multipliers used by the now-weighted `pickSegment`; boosted segments may also appear one tier above local difficulty), `gapB`/`wB` (gap and platform-width bias applied in `connect` — gaps capped at `256*gapK` so reachability holds), `rest` (breather-cadence offset), `elev` (macro elevation arc: flat/ascend/descend/valley/wave via `ELEV_SHAPES` — `segRest` steers 50% and `segGapRun` 35% toward `b.elevY()` so levels have a height storyline), and optional `needs` (segment ids the archetype requires — MACHINEWORKS is skipped on terrain stages whose pool lacks movers). Selection is deterministic per (stage, level) from an independent seed stream (`mulberry32(stage*50+lvl+9137)`) with a no-immediate-repeat rule. Levels also get ±25% seeded length variance (`lenVar` on `baseW`). The archetype name is exposed as `levelData.flavor` and shown in the HUD (`updateHUD(..., flavor)` appends it to `#hud-stage`, e.g. "S2-L13 · SPIKE FIELDS"); boss levels have `flavor:null`.
...
**Per-stage gameplay twists (`STAGE_MODIFIERS[]` — defined in level.js, re-exported by player.js so the level generator can derive each stage's jump envelope without an import cycle):** each stage applies a physics/visual modifier, selected via `setStageModifier(stageIdx)` (called in `startLevel` AND `respawnPlayer` in main.js, before `initPlayer`); `getStageModifier()` exposes the active one. Fields: `gMul` (gravity ×), `jMul` (jump/double-jump force ×, applied in `initPlayer`), `fric` (idle horizontal damping per frame — base 0.72; higher = slippery, lower = grippy), `wind` (oscillating sideways gust accel px/s², added to `vx` in `updatePlayer`), `dark` (1 = limited-visibility vignette drawn around the player in `renderFrame`). The twists: 0 Meadow 🌱 springy (jMul 1.12), 1 Cave 🕯️ pitch dark (vignette), 2 Icy Peaks ❄️ slippery (fric 0.965), 3 Desert 🌬️ gusts (wind 1300), 4 Lava 🔥 scorching (gMul+jMul 1.25 = heavy/snappy), 5 Sky ☁️ updrafts (gMul 0.6 floaty), 6 Forest 🍄 mossy grip (fric 0.42), 7 Space 🚀 zero-g (gMul 0.32), 8 Crystal 💎 bouncy (jMul 1.24), 9 Dark Fortress 🏰 heavy gravity (gMul+jMul 1.45). **Level geometry matches the twist:** the generator scales gaps/rises to each stage's real jump envelope (see Physics constants above), so floaty stages get proportionally wider gaps instead of trivial ones and every level stays solvable. `updatePlayer` uses `grav = GRAVITY*gMul` and `term = TERMINAL_VEL*max(0.45,gMul)`. The active twist's label shows in the HUD (`#hud-modifier`).
...
- **Fortress map**: static map (size=250) in map.js `buildFortressMap(obs)`; outer walls at ±85 (h=6) with 3 secret passages (N x=35, E z=10, W z=-35) — the old 4 outer-wall tunnel doors were removed (solid wall there now); **underground network**: `gameState.undergroundZones[]` (rectangular `{minX,maxX,minZ,maxZ,depth,entry?}`) drives `getFloorHeight` below y=0, but the surface above is **sealed** — the lowered floor applies only when the entity is already below ground (feet < -0.3) or inside a zone's `entry` rect (the stairwells), so no fall-through from the courtyard; central chamber at x=-6 to x=6, z=-6 to z=6 (floor y=-8) with 4 columns, PointLight, torches; N/E/W corridors 6-wide (N/W/S/E far ends sealed with underground end caps); S corridor (x=52–58) T-junctions into east corridor; **the ONLY entrances** are hidden stairwells at the ends of the secret wall corridors: Corridor A (N wall interior x=-75–-15) floor ends at x=-22, 16 steps (0.8d × 0.5h) descend east to y=-8 and a connector (z=-83..-80) joins the north corridor through a west-wall opening (z=-83..-79); Corridor B (E wall interior, extended to z=-75–-12) floor ends at z=-22, 16 steps descend south and a connector (x=80..83) joins the east corridor through a north-wall opening (x=79..83); each secret corridor has 2 passage doors, end caps, noCollide roof, torches; inner keep walls (h=10) with S doorway only; `addPassage(x,y,z,w,h,d,label)` stores label; prompt shows `Press [E] to open ${label}`; `gameState.secretPassages[]` + `passThrough` skips collision; **wall staircases**: `addStaircase(x0,z0,dir)` helper — 12 steps × 0.5h, one per wall side
