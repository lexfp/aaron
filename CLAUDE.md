...
**Main menu effects (`ui.js` `setupMenuEffects`):** The `#main-menu` has a full-screen `<canvas id="menu-fx">` particle system (drifting motes, mouse-trail sparkles, confetti/emoji bursts via `burst()`), orbiting emoji ring (`.menu-orbit`), animated gradient/float title, and `--menu-hue` hue-rotate filter. A `.toy-box` row of interactive widgets: 🎲 dice (random face + emoji burst), 🎨 palette (random hue + toggle rainbow particles), 🎆 spark (confetti), 🐸 boop (cycles faces), 🪩 disco (toggles `.disco` strobe class). **Secret cheat (hidden):** the 🎲 dice toy looks like an ordinary "roll the dice" widget, but secretly counts rapid clicks — spam it 500× (each click within 600ms of the last) and it calls `completeAllLevels()` (state.js), then shows `#win-banner`, confetti storm, and screen quake. No meter or counter is shown — the trigger blends in with the other toys. Click *counting* is cheap on every click, but the visual feedback (face change, wiggle, confetti) is throttled to once per 70ms and the dice center is cached (no per-click `getBoundingClientRect`/reflow); `burst()` is capped at `MAX_PARTICLES=140` total — so spamming never lags.
...
**Enemies & combat (`entities.js`):** `levelData.enemies[]` (built by `placeEnemies()` in level.js) is cloned into a live `enemies[]` by `initEntities` (so respawn resets them). Types: **walker** (patrols its platform), **jumper** (hops, p≥0.25), **brute** (big, high-HP, p≥0.40), **flyer** (sine-floats in wide gaps, p≥0.12). Density + HP + type variety scale with `progress`; the start area (x<320) and exit platform stay enemy-free, and the very first level (p<0.012) has none. Player has no health bar — **side contact with an enemy = instant death** (same as spikes/falling); **stomping** (landing on an enemy from above) is safe, deals `STOMP_DAMAGE=2`, and bounces the player. `updateEnemies(dt, platforms, player)` returns `{playerHit}`.
