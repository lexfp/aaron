...
**Player special attacks (`SPECIAL_DEFS` in state.js, logic in entities.js):** 3 one-time-purchase abilities, extremely expensive (2000–5000 coins). Each has a per-level cooldown tracked in `player.specialCDs` (reset to 0 on level start). Keys Q/E/R are consumed via `consumeSpecial(idx)` in input.js; the game loop in main.js ticks CDs down and calls the use function when key is pressed and CD is zero. All 3 bypass boss shields (`e.shieldHp=0` before dealing damage). `SPECIAL_DEFS`/`SPECIAL_MAP`/`ownsSpecial`/`buySpecial` live in state.js (same pattern as weapons). HUD strip `#hud-specials` (bottom of canvas, `.special-pill`) shows owned specials with cooldown bar — hidden when none owned, managed by `updateSpecialsHUD(specialCDs)` in ui.js. Shop has a "⚡ Special Attacks" section with `.special-card`/`.special-buy` classes. The 3 specials: **Heal Surge** (💊, Q, 2000 coins, 40s CD) — restores 45 HP + green particles; **Mega Bomb** (💣, E, 3500 coins, 25s CD) — drops a physics bomb with 2s fuse, 250px blast radius, 70 dmg (falloff from center), fuse spark flashes faster near 0; **Nova Strike** (🌀, R, 5000 coins, 30s CD) — instant screen-wide burst, 35 dmg to all enemies / 70 vs bosses + explosion particles on each hit. `playerBombs[]` array (entities.js) stores live bombs; `updatePlayerBombs(dt, platforms)` + `drawPlayerBombs(ctx, camX, GW)` handle physics + rendering. `initEntities` clears `playerBombs` on level start/respawn.
