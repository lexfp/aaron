...
class TopDogMilestonePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Top Dog Milestones', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: BlocBuilder<DogClickerBloc, DogClickerState>(
        builder: (context, state) {
          if (state is DogClickerLoaded) {
            final reachedCount = state.milestones.where((m) => m.isReached).length;
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: milestone.isReached ? Colors.amber : Colors.transparent,
                            width: 2,
                          ),
                        ),
                        child: ListTile(
                          ...
                          leading: CircleAvatar(
                            backgroundColor: milestone.isReached ? Colors.amber.withOpacity(0.2) : Colors.white10,
                            child: Icon(
                              milestone.isReached ? Icons.check : Icons.lock,
                              color: milestone.isReached ? Colors.amber : Colors.white54,
                            ),
                          ),
                          title: Text(
                            milestone.title,
                            style: TextStyle(
                              ...
                              fontSize: 18,
                              color: milestone.isReached ? Colors.white : Colors.white54,
                            ),
                          ),
                          subtitle: Text(
                            'Reach ${milestone.requiredScore} Bones',
                            style: TextStyle(
                              color: milestone.isReached ? Colors.white70 : Colors.white38,
                            ),
                          ),
                        ),
                      );
